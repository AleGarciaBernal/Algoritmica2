...
int maxFlow(int s,int t) { 
    ...
    while(true) { ///  nunca sabemos si no tenemos flujo
        ...
        queue<int> Q;
        Q.push(s);//agregamos s a la cola
        path[s] = s;  // agregamos S a nuestro camino
        while(!Q.empty() && path[t]==-1) { //recorrer mientras no este vacia, y t no la hayamos visitado
            int currentNode = Q.front(); //preguntamos el nodo actual
            Q.pop(); //sacamos de la cola
            for(int i = 0 ; i < MAX_N ; i++) { //"recorremos" a los vecinos del nodo actual, los conocemos
                if(grafo[currentNode][i] > 0 && path[i]==-1)  {  // puedo visitar? (capacidad >0 y que no hayamos visitado aun)
        ...
        for(int from = path[t],to = t; from!=to ; from = path[to = from]) {
            minFlow = min(minFlow,grafo[from][to]); // sacamos el minimo de peso de las aristas que pasamos
        }
