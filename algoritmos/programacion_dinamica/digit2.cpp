...
*/ 
int solve_dp(int pos, int mayor,int impares) {
    if(pos > number.size()) { // cuando la posicion exceda al numero dado
        return 0;
    }
    // Modificar de acuerdo al problema
    if(pos == number.size()) {
        if(impares>=2) { // la suma de los digitos debe ser divisible por 9
            return 1;
        }
        else {
            return 0;
    ...
    if(dp[pos][mayor][impares] == -1) { // Pregunto si no lo he calculado 
        int tope = 9;
        ...
        for(int digito = 0; digito <= tope; digito++) {
            if(digito == tope ) {
                dp[pos][mayor][digito] += solve_dp(pos+1, true, impares+digito%2==0);
            }
            else { // 0 1 2 
                dp[pos][mayor][digito] += solve_dp(pos+1, false, impares+digito%2==0);
            }
        }
    }
    return dp[pos][mayor][impares];
}
